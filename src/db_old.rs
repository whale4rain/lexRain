...
impl Database {
    ...
    // Get word by spelling from ECDICT
    pub fn get_word_by_spelling(&self, spelling: &str) -> Result<Option<Word>> {
        let word = self.dict_conn.query_row(
            "SELECT id, word, phonetic, definition, translation, pos, collins, oxford, tag, bnc, frq, exchange
             FROM stardict WHERE word = ?1 COLLATE NOCASE",
            params![spelling],
            |row| {
                Ok(Word {
                    id: Some(row.get(0)?),
